...
This isn't hypothetical — it already happened once. The first real CI run (PR #4) failed `verify-browser-packages.sh` in under 2 minutes: `WPEWEBKIT`/`COG`/`COG_PLATFORM_DRM` didn't resolve, while every Mesa/DRM/EGL line resolved fine. Root cause, confirmed against Buildroot's actual `package/wpewebkit/Config.in`: WPEWebKit's own EGL implementation has a hard dependency on an EGL-capable Wayland backend (`libwayland-egl`), which the defconfig never enabled at all — `BR2_PACKAGE_WAYLAND`/`BR2_PACKAGE_WAYLAND_PROTOCOLS` are now in the defconfig to fix that. This is a build-time library dependency of WPEWebKit's EGL code, not a runtime requirement — Cog's DRM backend still owns actual on-screen rendering via KMS; no compositor runs on the device. The "no X11/Wayland compositor" decision above still holds. Treat the next real Buildroot run as the actual test of whether that specific fix was enough, and the first real hardware pass as the test of everything else in this section — not this doc.
