...
Before each launch and after a detected crash, the supervisor reaps/kills the full Cage/Cog browser stack and clears stale `wayland-*` sockets/locks under its private `XDG_RUNTIME_DIR`. This is deliberately blunt because UTM/QEMU testing showed Cog can crash after interaction while Cage is still holding or cleaning up DRM master, leaving symptoms such as `wayland-0.lock`, `Device or resource busy`, `DRM_IOCTL_MODE_CREATE_DUMB failed: Permission denied`, and repeated swapchain allocation failures on the next launch. `TALARIA_BROWSER_CLEANUP_DELAY` (default 2s) gives the kernel/compositor a short settle window before retrying.
