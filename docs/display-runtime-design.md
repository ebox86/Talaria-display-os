...
Phase 1 (console bring-up, networking, `/data` mount, diagnostics logging), Phase 3's mode resolution, Phase 3's server-assignment polling, Phase 3's local pairing page, and Phase 3's browser supervision are all implemented under `external/board/talaria/display-x86_64/rootfs_overlay/`. The browser stack has been validated by real from-source CI builds and a VM-rendered browser test page. What's still unproven is physical target hardware and real Talaria dashboard/signage content.
...
- **Broad Mesa driver coverage** ([Browser Phase](#browser-phase)) with software fallbacks, instead of betting on any one GPU vendor's driver working.
...
| `TALARIA_DISPLAY_MODE` | no | `pairing` | One of `dashboard`, `signage`, `pairing`, `diagnostics`. A fresh/unconfigured device shows the local pairing page when the browser stack is healthy, and falls back to diagnostics if the browser cannot render. Any other value is treated as invalid — see [Fallback Behavior](#fallback-behavior). |
...
With that shape, an unassigned server can leave the display on the local pairing page, while a valid server response can switch it to `dashboard` or `signage` without a reflash. If the browser cannot render the pairing page, the browser supervisor falls back to local diagnostics.
...
S65talaria-hardware-report  write PCI/DRM/network report to /data/talaria
...
- `TALARIA_DISPLAY_MODE` set to anything other than `dashboard`, `signage`, `pairing`, or `diagnostics`.
...
- `EFFECTIVE_MODE` is `dashboard`, `signage`, or `pairing` and `DISPLAY_URL` differs from what's currently running → stop the old process (if any) and launch the browser against the new URL.
...
Before each launch and after a detected crash, the supervisor reaps/kills the full Cage/Cog browser stack and clears stale `wayland-*` sockets/locks under its private `XDG_RUNTIME_DIR`. `S80talaria-browser` does the same cleanup during init-level stop/start. This is deliberately blunt because UTM/QEMU testing showed Cog can crash after interaction while Cage is still holding or cleaning up DRM master, leaving symptoms such as `wayland-0.lock`, `Device or resource busy`, `DRM_IOCTL_MODE_CREATE_DUMB failed: Permission denied`, and repeated swapchain allocation failures on the next launch. `TALARIA_BROWSER_CLEANUP_DELAY` (default 5s) gives the kernel/compositor a short settle window before retrying.

The preferred browser path is Cage as a single-app Wayland compositor running Cog's Wayland platform:

- **Cage + Cog Wayland by default**: direct Cog DRM rendered the local browser test page, but VM testing against a real site exposed repeated crashes in `libcogplatform-drm.so` after redirects/repaints (`Buffer address requested when its parent pool has an external reference and a deferred resize pending`). Cog's alternate DRM renderer (`--platform-params=renderer=gles`) also crashed in the same test. The default is `cage -s -- cog --platform=wl <url>`, which keeps the appliance as a single full-screen browser while moving Cog off the crashing direct-DRM platform.
...
- **Tier 2 graphics coverage**: the x86_64 image includes Mesa softpipe, llvmpipe, Intel i915/crocus/iris, Radeon r300/r600/radeonsi, Nouveau, VMware SVGA, and virgl drivers. This lets modern and older desktop GPUs take their best supported path while retaining software fallback behavior. Per-device results still belong in `docs/hardware-inventory.md` because CI can prove package resolution and VM rendering, but not a random shop-floor GPU/monitor combination.

Browser subprocess output is appended to `/data/talaria/browser.log`; boot hardware facts are appended to `/data/talaria/hardware-report.log`. For renderer-specific experiments, the supervisor exposes `TALARIA_WLR_RENDERER` (for example `pixman`), `TALARIA_WLR_DRM_NO_MODIFIERS`, `TALARIA_MESA_LOADER_DRIVER_OVERRIDE` (for example `llvmpipe`, `i915`, `iris`, or `radeonsi`), and `TALARIA_BROWSER_FORCE_SOFTWARE_GL=1` as `/data/talaria/display.conf` overrides.
...
- **Renderer policy per hardware class** — blocked on real data, not a design decision: `docs/hardware-inventory.md` needs actual GPU/driver results from real machines before deciding whether specific models should prefer hardware GL, llvmpipe, or a wlroots renderer override.
...
- **Minimal/thin-client flavor**: old x86/x86_64 thin clients (HP t5xxx, Dell/Wyse) are architecturally the same platform as everything in `hardware-inventory.md`, but with RAM and flash an order of magnitude tighter than this image assumes (some as low as 512 MB RAM / 1 GB flash total, vs. this image's 3072M rootfs alone). Supporting that class for real means a genuinely smaller build variant — fewer packages, smaller rootfs, possibly a reduced driver set — not a defconfig tweak to the existing one. A second, separate defconfig (e.g. `talaria_display_x86_64_thin_defconfig`) is the likely shape, sharing the external tree and rootfs overlay scripts but not the package list.
